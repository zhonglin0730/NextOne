...
- 一个项目可以包含多个任务；
- 一个项目默认只突出一个 `focusTaskId`；
- 允许存在少量并行 READY 任务；
- 完成焦点任务后，系统建议下一项，不未经确认自动塞入今日计划；
- 没有焦点行动的活跃项目进入每周回顾队列。
...
| 一个项目一个下一步过于僵化 | 使用 focusTask，而非数据库唯一约束 |
...
- 产品定位：以项目下一步为中心、主动帮助用户做取舍的个人项目管理系统；
- 完整设计、阶段实施；
- 阶段 1：PC Web + Android；
...
- Web 一级导航固定为“项目 / 今天 / 收件箱 / 回顾 / 看板 / 设置”；项目驾驶舱是默认落点，看板是辅助视图；
