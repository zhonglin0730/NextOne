...
> 文档状态：M9.5 已完成，M10 部署配置已就绪，等待 Oracle ARM 实机验收
> 版本：0.1  
> 日期：2026-08-01
...
V0.1 的目标不是一次性完成完整产品，而是交付一个可以连续真实使用的个人项目推进闭环：
...
- Monorepo、Web、Spring Boot 服务端和 Expo Android 工程；
- Web 完整行动闭环、设置、数据安全、国际化与离线同步；
- PostgreSQL 迁移、OpenAPI 契约和自动化测试；
- Android 原生界面、SQLite 本地事务、Outbox 同步与 SecureStore 凭据；
- 本地开发 Docker Compose。

M9.5 项目驾驶舱与发布候选打磨已完成：以项目为默认主视角，Today 作为跨项目执行
切片，看板作为项目内辅助视图。M10 已补齐 Caddy HTTPS、生产 Compose、ARM64 构建、
备份恢复脚本、结构化日志和最小监控配置；仍需在 Oracle ARM 实机完成公网 HTTPS、
镜像构建、资源占用和备份恢复验收。
...
- 已实现独立工作包实体和最多两级的项目结构，工作包不进入任务状态机；
- 已实现全部进行中任务、第一项待开始、其他候选、等待中、最近完成和最近推进记录；
- 当前执行由任务状态实时派生，不保存额外焦点字段；开始任务时会自动加入今天；
- 已实现无可执行任务项目识别和“需要决定”提示，不展示虚构完成百分比；
...
- 已完成回顾中心，覆盖停滞、等待超时、长期进行中、无可执行任务项目、临期和再看日期到期规则；
- “保持准备就绪”会更新 `reviewedAt` 并写入 `REVIEWED` 事件，刚回顾过的事项不会立即再次进入停滞队列；
- 已完成基础行动日志，保留完成、放弃、回顾和项目执行记录；
...
- 已实现 Task、Project、WorkPackage、DailyPlan、Review 和 Event 服务端模块，服务端重新校验任务状态机、DOING 上限、今日承诺上限、工作包归属和 revision；
...
- 已实现与设计稿一致的通用设置、数据与安全、同步三级设置导航和双栏信息结构；
- 已实现语言、时区、日期格式、每周起始日、时间格式和主题设置，语言选择在刷新后保留；
- 已实现今日焦点、进行中、停滞和等待过久规则，并由应用层在每次决策前读取最新值；
- 已实现带 `schemaVersion` 的完整 JSON 导出、文件校验、数据量预览和明确覆盖确认；
- 导入和恢复前会自动保存当前本地数据，IndexedDB V4 独立保存最多可见的恢复点历史；
- 已实现“清除此设备的本地副本”，只清理 IndexedDB、缓存和本地设置，不调用任何云端删除接口；
- 已通过 Flyway V4 和 `/api/v1/account/deletion-requests` 提供 30 分钟有效的删除申请，接口只进入最终确认等待状态，不执行账户删除；
- 已继续使用 `zh-CN` 和 `en-XA` 伪本地化资源，所有新增界面文案均通过语言键读取；
- 已通过 32 项 TypeScript 测试、全量类型检查和生产构建，以及 9 项服务端测试（含 6 项真实 PostgreSQL 集成测试）；
- M7 仍只需本机环境，不需要 Oracle 服务器。
...
- 单步今日规划，从收件箱和看板选择最多三项任务；
- 安排前直接展示任务预估和所选任务的每日容量；
- Today 页面持续显示去重后的今日预计投入；
- 待确认、宽松、合适、偏满、超载容量反馈；
- DOING 任务的 Zen 单任务模式；
- 每日专注容量偏好；
- 今日规划、容量计算和 Zen 模式的国际化与无障碍路径。

验收：

- 今日规划始终允许取消，不阻断进入 Today；
- 选择超过三项时不会继续增加；
- Today 与 DOING 重复出现的任务不会重复累计时长；
- 未填写预估的任务会明确提示，不按零时长伪装成轻松计划；
- Zen 模式只显示当前任务、预估、完成、暂停和退出；
- Escape 可以退出晨间规划与 Zen 模式；
- 不新增外部日历、计时服务或付费依赖。
