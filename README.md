...
帮助个人在项目组合中看清整体进度和卡点，再进入项目看板组织执行流、用“今天”
...
客户端。现有能力包括项目组合、可选的工作包拆解、项目执行进度、执行看板、快速记录、今天、
收件箱、轻量今日规划、每日收尾、基础回顾、Zen 单任务模式、IndexedDB / SQLite 本地存储、
机会性自动同步和 SecureStore 安全凭据。产品和开发文档位于 `docs/`。

## 核心工作流

1. 创建项目，写清希望达成的结果；
2. 直接在项目看板创建可执行任务；任务较多时，再用工作包按阶段或成果分组；
3. 开始任务时自动加入今天，也可以先挑选少量任务安排到今天；
4. 完成、暂停或标记等待，让项目进度和卡点保持真实；
5. 暂停时可留一句“下次从哪里继续”；再次打开项目，选择有继续记录的任务接着做；
6. 通过每日收尾和周期回顾处理等待、停滞和没有可执行任务的项目，复用已有继续记录。

Web 专注模式支持番茄钟、短休息与起身活动提醒，专注次数和时长记录在任务中。
继续记录是用户填写的可选上下文，包含修改时间，与任务一起离线保存和同步。
它不会创建额外任务、改变项目优先级或自动安排今天；点击“继续这项任务”才开始执行并加入今天。
取消暂停不改变任务；“直接暂停”保留原继续记录。清空后保存可删除继续记录。
...
- 当前已实现 Web 工程基线、本地任务内核、项目组合、可选工作包结构、项目执行进度、执行看板、今天页、每日收尾与基础回顾；
...
- M9 已提供轻量今日规划、每日容量反馈和 Zen 单任务模式；
